...
const messages = {
  ...
  getDefaultDescription(
    ...
    let targetLanguageName =
      targetLanguage && langNames[targetLanguage]
        ? langNames[targetLanguage].name
        : "";
    let sourceLanguageName =
      sourceLanguage && langNames[sourceLanguage]
        ? langNames[sourceLanguage].name
        : "";

    let translate = `<match>${__("translate").capitalize()} </match>`;
    let match = `<match><dim>${
      query.length ? query.trunc(25) + " " : ""
    }</dim></match>`;

    let fromSource = sourceLanguageName
      ? `${__("from")} <url>${sourceLanguageName}</url> `
      : ``;
    let fromDetected = sourceLanguageName
      ? `<dim>(${sourceLanguageName}-ᴅᴇᴛᴇᴄᴛᴇᴅ)</dim> `
      : "";
    let from = isDetectedSourceLanguage ? fromDetected : fromSource;

    let toPrefix =
      sourceLanguage && !isDetectedSourceLanguage
        ? `${__("into")} `
        : `${__("in")} `;
  ...
  getSuggestDescription(
    ...
    let matchedLanguage = `<url><match>${languageName.slice(
      0,
    ...
    let matchedLang = `<match>${langName.slice(
      0,
      langSlice.length
    )}</match>${langName.slice(langSlice.length, langName.length)}`;
    let matchedAlias = aliasName
      ? `<match>${aliasName.slice(
          0,
          aliasSlice.length
        )}</match>${aliasName.slice(aliasSlice.length, aliasName.length)}`
      : "";
  ...
  getFromSuggestDescription(
    ...
    return `<match><dim>?</dim></match>${space.repeat(2)}${__(
      "translateFrom"
    )} <url><match>${languageName.slice(
      0,
      languageSlice.length
    )}</match>${languageName.slice(
      languageSlice.length,
      ...
      !aliasSlice ? langName.slice(0, langSlice.length) : ""
    }</match>${
      !aliasSlice ? langName.slice(langSlice.length, langName.length) : ""
    }<match>${aliasSlice ? aliasName.slice(0, aliasSlice.length) : ""}</match>${
      aliasSlice ? aliasName.slice(aliasSlice.length, aliasName.length) : ""
    })</dim>`;
  },
  ...
  getFromToSuggestDescription(source, target) {
    ...
    let matchedSourceLanguage = `<url><match>${source.languageName.slice(
      0,
      source.languageSlice.length
    )}</match>${source.languageName.slice(
      source.languageSlice.length,
      source.languageName.length
    )}</url> `;
    let matchedSourceLangOrAlias = `<dim>(<match>${
      !source.aliasSlice
        ? source.langName.slice(0, source.langSlice.length)
        : ""
    }</match>${
      !source.aliasSlice
        ? source.langName.slice(source.langSlice.length, source.langName.length)
        : ""
    }<match>${
      source.aliasSlice
        ? source.aliasName.slice(0, source.aliasSlice.length)
        : ""
    }</match>${
      source.aliasSlice
        ? source.aliasName.slice(
            source.aliasSlice.length,
            source.aliasName.length
          )
        : ""
    })</dim> `;
    let matchedSource = `${matchedSourceLanguage}${matchedSourceLangOrAlias}`;
    let into = `${__("into")} `;
    let matchedTargetLanguage = `<url><match>${target.languageName.slice(
      0,
      target.languageSlice.length
    )}</match>${target.languageName.slice(
      target.languageSlice.length,
      target.languageName.length
    )}</url> `;
    let matchedTargetLangOrAlias = `<dim>(<match>${
      !target.aliasSlice
        ? target.langName.slice(0, target.langSlice.length)
        : ""
    }</match>${
      !target.aliasSlice
        ? target.langName.slice(target.langSlice.length, target.langName.length)
        : ""
    }<match>${
      target.aliasSlice
        ? target.aliasName.slice(0, target.aliasSlice.length)
        : ""
    }</match>${
      target.aliasSlice
        ? target.aliasName.slice(
            target.aliasSlice.length,
            target.aliasName.length
          )
        : ""
    })</dim>`;
  ...
  getTranslationDescription(translatedText, translateService) {
    ...
    let space = `  `;
    return `${space}<match><dim>=</dim></match>${space}<url><match>${translatedText}</match></url><dim>${space.repeat(
      2
    )}${__("translatedBy__smallcase")} ${translateService.smallcase}</dim>`;
  },
  ...
  getLanguageSuggestDefaultDescription() {
    return `<match>${__("translateIn")}…</match> <dim>(${__(
      "selectWithArrowKeys"
    )}):</dim>`;
  },

  getWeirdResultMessage(sourceLanguage, targetLanguage) {
    return `<match>⚠</match> <dim>Same translation between source (<dim>${langNames[sourceLanguage].name}</dim>) and target (<dim>${langNames[targetLanguage].name}</dim>)</dim>`;
  },

  getLangOption(lang) {
    let option =
      langUtils.getDefaultLang() === lang
        ? "(default)"
        : langUtils.getSecondaryLang()
        ? "(secondary)"
        : "";
    return option;
  },

  getSwitchToSecondaryLanguageMessage(query, targetLanguage) {
    return `<match>⚠</match> Did you mean <dim><match>${query}</match></dim> in <match><url>${
      langNames[targetLanguage].name
    }</url></match>? <dim>${messages.getLangOption(targetLanguage)}</dim>`;
  }
};
